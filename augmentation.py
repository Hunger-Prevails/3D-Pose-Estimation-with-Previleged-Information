...
def augment_brightness(image, colorspace):
    if colorspace != 'rgb':
        image = cv2.cvtColor(image, cv2.COLOR_HSV2RGB)

    image += np.random.uniform(-0.1, 0.1)

    return 'rgb', np.clip(image, 0.0, 1.0)


def augment_contrast(image, colorspace):
    if colorspace != 'rgb':
        image = cv2.cvtColor(image, cv2.COLOR_HSV2RGB)

    image -= 0.5
    image *= np.random.uniform(4.0 / 5.0, 5.0 / 4.0)
    image += 0.5
    
    return 'rgb', np.clip(image, 0.0, 1.0)


def augment_hue(image, colorspace):
    if colorspace != 'hsv':
        image = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)

    image[:, :, 0] += np.random.uniform(-72, 72)
    image[:, :, 0] %= 360
    
    return 'hsv', image


def augment_saturation(image, colorspace):
    if colorspace != 'hsv':
        image = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)

    image[:, :, 1] *= np.random.uniform(2.0 / 3.0, 3.0 / 2.0)
    image[:, :, 1] = np.clip(image[:, :, 1], 0.0, 1.0)
...
def augment_color(image):
    ...
    augment_functions = [augment_brightness, augment_contrast, augment_hue, augment_saturation]

    np.random.shuffle(augment_functions)

    colorspace = 'rgb'
    for function in augment_functions:
        colorspace, image = function(image, colorspace)
...
def augment_object(image, occ_idx, occ_path):
