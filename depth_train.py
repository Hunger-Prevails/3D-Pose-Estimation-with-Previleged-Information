...
class Trainer:
    ...
    def depth_train(self, epoch, data_loader, cuda_device):
        ...
        for i, (color_image, depth_image, true_cam, valid_mask) in enumerate(data_loader):
            ...
            if self.n_cudas:
                depth_image = depth_image.to(cuda_device)

                true_cam = true_cam.to(cuda_device)

                valid_mask = valid_mask.to(cuda_device)

            batch = depth_image.size(0)

            cam_feat = self.model(depth_image)
    ...
    def train(self, epoch, data_loader, comp_loader):
        ...
        return self.depth_train(epoch, data_loader, torch.device('cuda'))


    def depth_test(self, epoch, test_loader, cuda_device):
    ...
    def test(self, epoch, test_loader):
        self.model.eval()
        return self.depth_test(epoch, test_loader, torch.device('cuda'))
